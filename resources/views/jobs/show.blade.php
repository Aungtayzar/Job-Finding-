    ...
    <div class="grid grid-cols-1 md:grid-cols-4 gap-6">
            ...
            <div class="rounded-lg shadow-md bg-white p-3">
                <div class="flex justify-between items-center">
                    ...
                    <div class="flex space-x-3 ml-4">
                        ...
                        >
                        <!-- Delete Form -->
                        <form method="POST" action="{{route('jobs.destroy',$job->id)}}" onsubmit="return confirm('Are you sure you want to delete!')">
                            @csrf
                            @method('DELETE')
                            <button
                                type="submit"
                    ...
                    <div x-cloak x-show="open" class="fixed inset-0 flex items-center justify-center bg-gray-900 bg-opacity-50">
                        <div @click.away="open = false" class="bg-white p-6 rounded-lg shadow-md w-full max-w-md">
                            ...
                            <form action="{{route('applicant.store',$job->id)}}" method="POST" enctype="multipart/form-data">
                                ...
                                <x-inputs.text id="email" name="contact_email" label="Contact Email" :required="true" />
        ...
        <aside class="bg-white rounded-lg shadow-md p-3">
            ...
            <p class="mt-10 bg-gray-200 text-gray-700 font-bold w-full py-2 px-4 rounded-full text-center">
                <i class="fas fa-info-circle mr-3"></i> You must be logged in the bookmark a job 
            </p>
            @else 
                <form action="{{auth()->user()->bookmarkedJobs()->where('job_id',$job->id)->exists() ? route('bookmarks.destroy',$job->id) : route('bookmarks.store',$job->id)}}" method="POST" class="mt-10">
                    @csrf
